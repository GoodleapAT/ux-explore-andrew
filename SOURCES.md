...
| **Cowork artefacts, commenting** | **Settled 21 Sep 2026, and the answer is that it does not work. Andrew retested with the Artifacts capability switched on and still could not comment on an artefact**, which removes the confound the first test had and makes the 16 Sep result stand rather than overturning it. **Do not suggest a settings change as the fix again.** The original test on 16 Sep was run with the capability off in Settings, Capabilities, Visuals, which looked like the cause because commenting is a collaboration feature and the capability covers publishing to a shareable page. It was not the cause. **This is one of the two reasons Andrew moved the work to Claude Code**, where commenting on an artefact does work. What has not been tested is whether a Cowork artefact that has been published and shared behaves differently from one that has not |
