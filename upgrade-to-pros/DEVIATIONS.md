...
| **The project is created on 4 August, when the app contact qualifies itself.** S15 records a 12 August pursuit decision as an internal decision with no writes, and mints its spine at the first proposal. **Moved three times on 15 Sep**, finally to the first contact | It is the sharpest claim in our model and the moment the container exists for. The gap is one beat wide | **Proposal** | Open. This is the live disagreement with DRAFT v3 and is already recorded as a decision |
...
## S15 demand story, 15 Sep 2026, built against S15 as at 4 Sep 2026

**Scenario last checked for drift: 13 Sep 2026, unmoved.** This is a change to the worked example
rather than to a mock, and it is recorded here because the next mock built from that data inherits it.

| What deviates | Why | Kind | Resolution |
|---|---|---|---|
| **The customer raises one trade, not two.** S15 has the roof raised through the Home App and the siding raised by the customer on a follow-up call. Here the roof is the only customer contact | The siding becomes an **opportunity**, recommended by the contractor's own system on photographic evidence from a job it did a year earlier. **The customer's words survive**, as an answer on the intake call rather than the reason for it | **Proposal** | Open. This one should reach the team, because it is a better story than the script's and it names an object neither model has |
| **All three trades are opportunities, and the trade is a durable record.** S15 has three trades inside one piece of demand, living and dying with it | **The biggest departure, and it should reach the team first.** A trade a customer might buy is one record per customer, per property, per trade, with origin as an attribute. **It outlives the lead**, so a declined roof stays visible under the customer with its whole history instead of being a note on something that closed. **Their own scenarios handle that case in prose rather than in the model**, which is the argument to make | **Proposal** | Open. **Two earlier reasons for this row have been withdrawn**: homelessness, which the qualification change dissolved, and the opportunity-versus-interest distinction, which the retirement of the interest replaced. **Send it on the durability argument alone** |
| **Qualification is automatic and the lead exists from the first contact** | S15 treats its 12 August pursuit decision as the moment the work begins. Here the contact names a trade and a property, so the lead and the project exist from **4 August**, and **10 August is Start selling instead**. The container is then thin for six days | **Proposal** | Open. **The largest one in this table and the one most likely to interest the team**, because their own chain has qualification as an act and this makes it a consequence |
| **The demand grain count is unchanged at four, having been five in between** | The opportunity was added above the interest, then the interest was retired and the opportunity took over its status and history. **So the net change is one object swapped for a better one**, not a grain added | **Proposal** | Open. **The four are now different jobs rather than different gradings**, which is a stronger position than the one this row originally recorded |
| **The front of the story is compressed from twenty two days to six** | S15 runs demand from 21 July to 12 August. Ours runs 4 to 10 August, with everything from 13 August onward untouched. **Andrew's instruction**, because a lead and project sitting for three weeks was a screen problem nobody wanted to solve. **What it costs:** the gap was the thing that made S15 test Start selling as an act separate from qualification, and the contrast with S14's two minutes drops from three orders of magnitude to two | **Simplification** | Open. **It is a simplification of the scenario to suit the design**, which is the direction this register exists to catch |
| **Northgate gains a CSR, and the intake call moves to her** | S15 has one person throughout. **Renata Silva takes the 5 August call and hands the lead to Priya**, which is the S14 shape. Andrew's instruction. **What it costs:** Northgate is now organised like Brightline, so one axis of difference between the two scenarios goes, **and the hand-off is a beat neither scenario had drawn** | **Simplification** | Open. **Worth asking whether the team wants both scenarios run by the same shape of organisation**, because the pair was chosen partly for the contrast |
| **The 2025 heat pump install gains site photographs** | The siding recommendation needs evidence, and photographs taken for the equipment record are the cheapest plausible source. S15 says nothing about the prior job at all | **Simplification** | Open. Low risk, but it is invented |
